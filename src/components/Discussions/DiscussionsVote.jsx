...
export default function DiscussionsVote({ upVotesBy, downVotesBy, isDetailDiscuss = false }) {
  ...
  return (
    <div className="flex flex-row">
      <div className="flex cursor-pointer" onClick={handleUpVoteClick}>
        ...
        <span className="ms-1 my-auto text-4 fw-500 mt-0.5">{upVotesBy?.length}</span>
      </div>
      <div className="flex ms-4 cursor-pointer" onClick={handleDownVoteClick}>
        {isDownVoted ? (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="1.25em"
        ...
        <span className="ms-1 my-auto text-4 fw-500 mt-0.5">{downVotesBy?.length}</span>
      </div>
    </div>
  )
}
...
DiscussionsVote.propTypes = {
  ...
  isDetailDiscuss: PropTypes.bool,
}
